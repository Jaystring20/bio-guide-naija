...
export function generatePDF(data: PDFData, returnBlob?: boolean): { blob: Blob | null; fileName: string } {
  ...
  addText("Generated by BioGuide — bioguide-naija.lovable.app", margin, 7, { color: [150, 150, 150] });

  const fileName = `BioGuide-Report-${new Date(data.uploadDate).toLocaleDateString("en-NG", { day: "numeric", month: "short", year: "numeric" }).replace(/\s/g, "-")}.pdf`;
...
export async function sharePDF(data: PDFData, method: "whatsapp" | "email" | "native") {
  ...
  if (method === "native" && navigator.share && blob) {
    try {
      ...
      await navigator.share({
        title: "BioGuide Lab Report",
        text: "Check out my lab report from BioGuide",
        files: [file],
      });
      return true;
    } catch (e: any) {
  ...
  if (method === "whatsapp") {
    ...
    const text = encodeURIComponent(
      "Here's my lab report from BioGuide 🩺\nDownload the PDF I just shared and check it out!\n\nhttps://bio-guide-naija.lovable.app"
    );
  ...
  if (method === "email") {
    ...
    const subject = encodeURIComponent("My BioGuide Lab Report");
    const body = encodeURIComponent(
      "Hi,\n\nPlease find my lab report from BioGuide attached (downloaded separately).\n\nGenerated by BioGuide — https://bio-guide-naija.lovable.app"
    );
