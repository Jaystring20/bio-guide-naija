import { ArrowUp, ArrowDown, Minus, AlertCircle, CircleOff, Sparkles } from "lucide-react";
...
const VERDICT_STYLES: Record<string, { label: string; cls: string }> = {
  ...
  unchanged: { label: "About the same", cls: "bg-muted text-muted-foreground border-border" },
  new: { label: "First time measured", cls: "bg-accent/15 text-secondary-foreground border-accent/30" },
  dropped: { label: "Not in newer report", cls: "bg-muted text-muted-foreground border-border" },
  unit_mismatch: { label: "Units differ", cls: "bg-[hsl(var(--alert-amber))]/15 text-[hsl(var(--alert-amber))] border-[hsl(var(--alert-amber))]/30" },
};
...
export const BiomarkerDeltaCard = ({ delta, aLabel, bLabel, highlight }: { delta: PairDelta; aLabel: string; bLabel: string; highlight?: boolean }) => {
  ...
  const nonComparable = delta.verdict === "dropped" || delta.verdict === "new" || delta.verdict === "unit_mismatch";
  const arrow =
  ...
  return (
    <div className={`rounded-2xl border p-4 shadow-soft ${highlight ? "border-accent bg-accent/5" : "border-border bg-card"}`}>
      <div className="flex items-start justify-between gap-3 mb-3">
        ...
        <span className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-bold whitespace-nowrap ${v.cls}`}>
          {v.label}
        </span>
      </div>
      ...
      ) : delta.verdict === "dropped" ? (
        <div className="mt-1 flex items-start gap-1.5 rounded-lg bg-muted px-2.5 py-2 text-[11px] text-muted-foreground">
          <CircleOff className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
          <span>This marker wasn't included in the newer report — nothing has "dropped" clinically.</span>
        </div>
      ) : delta.verdict === "new" ? (
        <div className="mt-1 flex items-start gap-1.5 rounded-lg bg-accent/10 px-2.5 py-2 text-[11px] text-secondary-foreground">
          <Sparkles className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
          <span>Measured for the first time — no baseline to compare against yet.</span>
        </div>
      ) : !nonComparable && delta.abs !== null && delta.pct !== null ? (
...
const ValueCell = ({ label, value, raw, status, unit, newer }: { label: string; value: number | null; raw?: string | number; status?: string; unit: string; newer?: boolean }) => {
  const missing = value === null && (raw === undefined || raw === null || raw === "");
  const statusCls =
  ...
  return (
    <div className={`rounded-xl border ${newer ? "border-accent/30 bg-accent/5" : "border-border bg-muted/30"} ${missing ? "opacity-70" : ""} px-3 py-2`}>
      <p className="text-[10px] font-semibold uppercase tracking-wide text-muted-foreground truncate">{label}</p>
      {missing ? (
        <p className="font-display text-sm font-bold leading-tight text-muted-foreground italic">Not measured</p>
      ) : (
        <>
          <p className={`font-display text-lg font-extrabold leading-tight ${statusCls}`}>
            {value !== null ? formatNum(value) : String(raw)}
          </p>
